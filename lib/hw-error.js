...
function buildErrorType(name, opt) {
  ...
  function errorConstructor(message, extra) {
    /* jshint -W040: true */
    _.extend(this, _.omit(opt, ['parent', 'constructor', 'customize']));
...
that = {
  ...
  init: function () {
    ...
    _.forIn(that.defaultErrors, function (value, key) {
      logger.enabledLevels.debug && log.debug('building error type :', key);
    ...
    if (opt.declare !== false) {
      ...
      that[throwFnName] = function () {
        var args = Array.prototype.slice.call(arguments)
          , error;
        /* jshint -W058: true */
        logger.enabledLevels.debug && log.debug('throwing error ', errorClass.name);
